...
| 17 | video-hero              | home, full-width below header | x | x | x | · | **Built 2026-09-20, revised three times same day** (3-up row → prev/next/dots/sound carousel → arrow-only → CSS-default single slide, chasing "looks like a video player" then "shows as stacked videos" feedback — the latter turned out to be `evoearth.living` still pointing at the old Hostinger placeholder / a non-server preview where ES modules don't run, not a bug in the deployed code, but the fix — making single-slide the plain-CSS default instead of a JS-added state — makes it un-regressable either way). Final: single full-width stage (`js/modules/video-hero.js`), `.is-active` keyed directly in CSS so slide 1 is the only one visible even before/without JS; auto-advances on `ended`; **only** control is one next-arrow, shipped `hidden` in markup and un-hidden by JS (never shown if JS doesn't run). No-JS fallback: exactly one clip (Dry Amenities), `autoplay muted loop`, no arrow, real `<a>` link — the other two slides sit inert at `preload="none"`. Posters are a plain dark radial panel + italic kicker (no play-button icon). Kraft hairline border top/bottom keeps the framed-media language on an edge-to-edge element. `prefers-reduced-motion` never autoplays; next-arrow still switches slides. Playwright-verified: JS-off (confirmed only slide 1 visible/interactive, arrow stays hidden), reduced-motion, synthetic `ended` auto-advance, next-arrow click, 1440 (`21/9`, capped 65vh) + 390 (`4/5`) layouts, axe 0 violations. **Blocked on client:** the actual video files |
...
| 19 | category-showcase       | home, below video hero | x | x | x | · | **Built 2026-09-20.** 6-tile "shop by category" grid (`.showcase`/`.showcase__grid` in `components.css`) placed directly below the video hero (spec 17), above the existing hero/about flow — the default position named in the spec, since the client's brief cut off before confirming otherwise. Square tiles, image + gradient scrim + label overlay (distinct treatment from spec 04's range cards: full-bleed caption-on-image vs. image-above-body). All 6 links resolve to real anchors — verified against actual rendered DOM ids, not the brief's guessed slugs: `#face-body` (not "face-and-body-care" as the brief assumed — `subcatSlug()` strips `&` rather than expanding it), `#scented-candles`, `#reed-diffuser`, `#room-freshener-spray`, `#soaps`; "Luxury Gift Boxes" points at `/products/gifting.html` (no anchor — no unified "luxury" section exists there yet, only the 4 occasion sub-sections with pending bundle content per spec 16). Images are the same `item.svg` placeholder used elsewhere pending spec 14 photography. Playwright-verified 390/1440: no h-scroll, 2-col mobile / 3-col desktop grid, axe 0 violations, console clean. Cache-buster bumped to `?v=24` (also fixed `main.css`'s `@import`s, which had drifted to `?v=18` against the HTML's `?v=23`). **Still open (per spec, blocked on client):** rest of the homepage brief after this section, and whether this section's position is meant to replace part of the existing hero/about order. |
| 20 | catalogue-gate          | all (14 `[data-catalogue]` CTAs across 5 pages) | x | x | ~ | · | **Built 2026-09-20.** New `js/modules/catalogue-gate.js` (modal open/close reuses spec 01's nav inert/scroll-lock/focus pattern; per-field blur+submit validation with normalised email/phone; Netlify Forms AJAX submit with honeypot; `localStorage` skip on repeat visits). `contact.js` no longer sets `href` on `[data-catalogue]` — click is fully owned by the gate now, closing the middle-click/ctrl-click bypass a live href would leave open. New `.field`/`.catalogue-gate` CSS in `components.css`, `.visually-hidden` added to `utilities.css`. Cache-buster bumped to `?v=26` (page-level) / `?v=12` (JS module imports). Playwright-verified on index + gifting: opens on click, inline validation clears/reflags correctly, error state renders on a non-2xx response (only reachable locally — Python's dev server can't do POST, so the real Netlify Forms round-trip is unverified until deployed), repeat-visit `localStorage` skip opens the PDF directly with the modal never shown. **Not yet run:** axe pass, full keyboard-only walk, all 5 pages, Netlify dashboard form registration after deploy |
| 21 | google-sheet-sync       | catalogue-gate modal (all 5 pages) | x | x | · | · | **Built 2026-09-20.** `SITE.leadSheetWebhook` (empty by default, `js/data/site.js`) + `syncToSheet()` in `catalogue-gate.js` fire a best-effort, `no-cors`, fire-and-forget POST of `{name, organisation, email, phone, page, submittedAt}` to a client-owned **Google Apps Script Web App** right after a successful Netlify submission — Netlify Forms stays the record of truth, this is a duplicate write into a sheet the client controls. Full client setup steps + the Apps Script `Code.gs` to paste are in `specs/21-google-sheet-sync.md`. **Blocked on client:** they need to create the Sheet, deploy the Apps Script Web App from their own Google account, and hand back the `/exec` URL to paste into `site.js` — no code work remains on our side until that URL exists. Not yet verified against a real deployed Apps Script endpoint (only reachable once the client's URL is in hand). |
...
5. ~~Spec 19 (category showcase) — build the 6 tiles now~~ — **done 2026-09-20**,
   see row 19 above. Revisit once the rest of the homepage brief (cut off) is
   supplied.
